#!/usr/bin/env python3
"""Build Table-1-style comparison for unified regular-gain experiments."""
...
def candidate_paths(run_dir: Path, *, dataset: str, bit_tag: str) -> list[tuple[str, Path]]:
    paths = [
        (
            "unified_regular_gain",
            run_dir / f"unified_regular_gain_{dataset}_turboquant_{bit_tag}_full.jsonl",
        )
    ]
    if bit_tag == "2p5":
        # At the 2.5-bit TurboQuant split, `regular_gain_mse` applies norm gain
        # to the 2-bit regular subchannels and MSE to the 3-bit outlier
        # subchannels. That is behaviorally equivalent to the earlier complete
        # `lowbit_gain_mse` runs, so use those as a completion fallback.
        paths.append(
            (
...
def load_best_candidate(run_dir: Path, *, dataset: str, bit_tag: str, expected: int) -> tuple[str, Path, dict[int, dict[str, Any]]]:
    loaded = []
    for source, path in candidate_paths(run_dir, dataset=dataset, bit_tag=bit_tag):
        rows = load_rows(path)
...
def summarize_bit(run_dir: Path, baseline_dir: Path, *, bit_tag: str, baseline_bit_tag: str, kv_bits: float) -> dict[str, Any]:
    task_rows = []
    for category, dataset, expected in TASKS:
        ...
        candidate_source, candidate_path, candidate_rows = load_best_candidate(
            run_dir,
            dataset=dataset,
            bit_tag=bit_tag,
            expected=expected,
        ...
        task_rows.append(
            {
                ...
                "delta": candidate_score - baseline_score if complete and baseline_score is not None and candidate_score is not None else None,
    ...
    return {
        "method": "Unified Regular-Gain TurboQuant",
        "baseline": "TurboQuant",
        "kv_bits": kv_bits,
        "bit_tag": bit_tag,
...
def write_csv(results: list[dict[str, Any]], path: Path) -> None:
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow(["KV bits", "Level", "Name", "Baseline", "Candidate", "Delta", "Cache ratio", "Complete"])
...
def write_md(results: list[dict[str, Any]], path: Path) -> None:
    ...
    for result in results:
        ...
        lines.extend(["", "| Category | Task | Records | Source | TurboQuant | Unified Regular-Gain | Delta | Complete |"])
        lines.append("| --- | --- | ---: | --- | ---: | ---: | ---: | --- |")
        for row in result["task_rows"]:
            records = f"{row['candidate_records']}/{row['expected']}"
            lines.append(
                f"| {row['category']} | `{row['dataset']}` | {records} | {row['candidate_source']} | {fmt(row['baseline_score'])} | "
...
def main() -> None:
    ...
    parser.add_argument("--output-prefix", default=str(PROJECT_ROOT / "reproduce/incremental/unified_regular_gain_table1"))
    args = parser.parse_args()

    run_dir = Path(args.run_dir)
    baseline_dir = Path(args.baseline_dir)
    results = [
        summarize_bit(run_dir, baseline_dir, bit_tag=bit_tag, baseline_bit_tag=baseline_bit_tag, kv_bits=kv_bits)
